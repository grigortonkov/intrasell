...
Public function OpenAusdruck_inWord_Filename_RTF(ByVal VorgangTyp, ByVal Vorgang_Nummer,  ByVal Dateiname)
   ...
   Set rs = openRecordset(getRecSource(VorgangTyp, Vorgang_Nummer), dbOpenDynaset)
    ...
    If checkIfWeitereVorhanden(rs("idnr"), VorgangTyp) And VorgangTyp = "LI" Then
           ...
           Set rs = CurrentDb.openRecordset(getRecSource_Weitere(VorgangTyp, Vorgang_Nummer), dbOpenDynaset)
           'End If
    End If
    
  'Anzahl Positionen
   sql = "select count(*) as pos from [" & vonForm_Artikel & "] where rechnr=" & Vorgang_Nummer
   Set rsArt = openRecordset(sql, dbOpenDynaset)
  ...
  Set rsEig = openRecordset(sql, dbOpenDynaset)
  While Not rsEig.EOF
